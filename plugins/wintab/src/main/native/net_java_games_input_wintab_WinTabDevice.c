...
#include <jni.h>
#include "net_java_games_input_wintab_WinTabDevice.h"
#include "util.h"
...
JNIEXPORT jstring JNICALL Java_net_java_games_input_wintab_WinTabDevice_nGetName(JNIEnv *env, jclass unused, jint deviceIndex) {
	char name[50];
	gpWTInfoA(WTI_DEVICES + deviceIndex, DVC_NAME, name);
	return (*env)->NewStringUTF(env, name);
}

JNIEXPORT jintArray JNICALL Java_net_java_games_input_wintab_WinTabDevice_nGetAxisDetails(JNIEnv *env, jclass unused, jint deviceIndex, jint axisId) {
	UINT type;
...
JNIEXPORT jobjectArray JNICALL Java_net_java_games_input_wintab_WinTabDevice_nGetCursorNames(JNIEnv *env, jclass unused, jint deviceId) {
...
JNIEXPORT jint JNICALL Java_net_java_games_input_wintab_WinTabDevice_nGetMaxButtonCount(JNIEnv *env, jclass unused, jint deviceId) {
	int numberCursorTypes;
	int firstCursorType;
	byte buttonCount;
	int i;
