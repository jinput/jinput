...
#include <jni.h>
#include "net_java_games_input_wintab_WinTabContext.h"
...
JNIEXPORT jlong JNICALL Java_net_java_games_input_wintab_WinTabContext_nOpen(JNIEnv *env, jclass unused, jlong hWnd_long) {
	LOGCONTEXT context;
...
JNIEXPORT void JNICALL Java_net_java_games_input_wintab_WinTabContext_nClose(JNIEnv *env, jclass unused, jlong hCtx_long) {
	gpWTClose((HCTX)(INT_PTR)hCtx_long);
	UnloadWintab();
}

JNIEXPORT jint JNICALL Java_net_java_games_input_wintab_WinTabContext_nGetNumberOfSupportedDevices(JNIEnv *env, jclass unused) {
	int numDevices;
	gpWTInfoA(WTI_INTERFACE, IFC_NDEVICES, &numDevices);
	return numDevices;
}

JNIEXPORT jobjectArray JNICALL Java_net_java_games_input_wintab_WinTabContext_nGetPackets(JNIEnv *env, jclass unused, jlong hCtx_long) {

	jobjectArray retval;
	int i=0;
	PACKET packets[MAX_PACKETS];
	int numberRead = gpWTPacketsGet((HCTX)(INT_PTR)hCtx_long, MAX_PACKETS, packets);
	jclass winTabPacketClass = (*env)->FindClass(env, "net/java/games/input/wintab/WinTabPacket");
	...
	jobject prototypePacket = newJObject(env, "net/java/games/input/wintab/WinTabPacket", "()V");
	
	retval = (*env)->NewObjectArray(env, numberRead, winTabPacketClass, NULL);
	for(i=0;i<numberRead;i++) {
		jobject tempPacket = newJObject(env, "net/java/games/input/wintab/WinTabPacket", "()V");
