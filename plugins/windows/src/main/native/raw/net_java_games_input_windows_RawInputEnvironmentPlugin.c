...
#include <regstr.h>
#include <jni.h>
#include "net_java_games_input_windows_RawInputEnvironmentPlugin.h"
#include "util.h"
#include "winutil.h"

JNIEXPORT jbyteArray JNICALL Java_net_java_games_input_windows_RawInputEnvironmentPlugin_getKeyboardClassGUID(JNIEnv *env, jclass unused) {
	return wrapGUID(env, &GUID_DEVCLASS_KEYBOARD);
}

JNIEXPORT jbyteArray JNICALL Java_net_java_games_input_windows_RawInputEnvironmentPlugin_getMouseClassGUID(JNIEnv *env, jclass unused) {
	return wrapGUID(env, &GUID_DEVCLASS_MOUSE);
}

JNIEXPORT void JNICALL Java_net_java_games_input_windows_RawInputEnvironmentPlugin_nEnumSetupAPIDevices(JNIEnv *env, jclass unused, jbyteArray guid_array, jobject device_list) {
	jclass list_class;
	...
	for (i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &DeviceInfoData); i++) {
		...
		setup_api_device = newJObject(env, "net/java/games/input/windows/SetupAPIDevice", "(Ljava/lang/String;Ljava/lang/String;)V", device_instance_id, device_name);
...
JNIEXPORT void JNICALL Java_net_java_games_input_windows_RawInputEnvironmentPlugin_enumerateDevices(JNIEnv *env, jclass unused, jobject queue, jobject device_list) {
	UINT num_devices;
	UINT res;
	...
	for (i = 0; i < num_devices; i++) {
		device = devices + i;
		device_object = newJObject(env, "net/java/games/input/windows/RawDevice", "(Lnet/java/games/input/windows/RawInputEventQueue;JI)V", queue, (jlong)(INT_PTR)device->hDevice, (jint)device->dwType);
