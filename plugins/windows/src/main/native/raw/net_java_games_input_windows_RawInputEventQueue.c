...
#include <windows.h>
#include <jni.h>
#include "net_java_games_input_windows_RawInputEventQueue.h"
#include "util.h"
...
JNIEXPORT void JNICALL Java_net_java_games_input_windows_RawInputEventQueue_nRegisterDevices(JNIEnv *env, jclass unused, jint flags, jlong hwnd_addr, jobjectArray device_infos) {
	BOOL res;
...
/*	res = GetRegisteredRawInputDevices(NULL, &num_devices, sizeof(RAWINPUTDEVICE));
	...
	device_info_class = (*env)->FindClass(env, "net/java/games/input/windows/RawDeviceInfo");
...
JNIEXPORT void JNICALL Java_net_java_games_input_windows_RawInputEventQueue_nPoll(JNIEnv *env, jobject self, jlong hwnd_handle) {
	MSG msg;
